...
async function initialLoading() {
    ...
    //});
    filter_questions = sortArrayRandomly(roam_data_array_me.questions);
...
function setEventListnersForQuestionSection() {
    ...
    document.querySelector(".tabs .search-icon").addEventListener("click", () => {
        debugger;

        var d = document.querySelector(".search-bar");
        if (d) {
            displayInMainPage("search");
            var x = document.querySelector(".search-result .active");
            if (x) x.scrollIntoView({ behavior: "smooth" });
            return;
        }
        var div = document.createElement("div");
        div.className = "search-bar me-d-flex-c";
        document.querySelector(".search-page").appendChild(div);
        div.innerHTML = `
        <div class="top me-d-flex">
            <input type="search" placeholder="search text" class="search-input" />
            <button class="search me-btn me-cp">search</button>
        </div>
        <div class="filter-sec me-d-flex">
            <span class=" filter me-link ncert">ncert</span>
            <span class=" filter me-link questions">questions</span>
            <span class=" filter me-link  normal">normal</span>
            <span class=" filter me-link  mcq">mcq</span>
            <span class=" filter  me-link pyq">pyq</span>
        </div>
        <div class="search-result"> </div>
        `;
        displayInMainPage("search");
        div.querySelectorAll(".filter").forEach((ele) => {
            ele.addEventListener("click", (e) => {
                var span = e.target;
                span.classList.toggle("active");
                div.querySelectorAll(".filter-sec .active").forEach((ee) => {
                    if (ee != span) ee.classList.remove("active");
                });

                if (!span.classList.contains("active")) {
                    document.querySelectorAll(".search-result > div").forEach((dd) => {
                        dd.classList.remove("hide");
                    });
                    return;
                }
                debugger;
                const hasItemInClassList = (classList, items) => items.find((item) => Array.from(classList).some((className) => items.includes(className)));
                var curr_item = "";
                const items = ["ncert", "questions", "pyq", "mcq", "normal"];
                items.forEach((item) => {
                    if (span.classList.contains(item)) curr_item = item;
                });
                //const foundItem = hasItemInClassList(span.classList, items);

                //const items = ["ncert", "questions", "pyq", "mcq", "normal"];
                //const foundItem = hasItemInClassList(span.classList, items);
                if (curr_item == "questions") {
                    document.querySelectorAll(".search-result > div").forEach((dd) => {
                        if (dd.classList.contains("ncert")) dd.classList.add("hide");
                        else dd.classList.remove("hide");
                    });
                    return;
                } else {
                    document.querySelectorAll(".search-result > div").forEach((dd) => {
                        if (dd.classList.contains(`${curr_item}`)) dd.classList.remove("hide");
        ...
        div.querySelector("button.search").addEventListener("click", (e) => {
            var search_text = div.querySelector("input").value.trim();
            if (search_text == "") {
                popupAlert("add some text in search input");
                return;
            }
            var target = div.querySelector(".search-result");
            target.innerHTML = "";
            // get all ncert blocks
            var pages = pages_data.pages;
            pages.forEach((page) => {
                var blocks = page.blocks;
                var page_uid = page.uid;
                blocks.forEach((block) => {
                    findSearchTextInBlocks(block, target, search_text, page_uid);
                });
            });

            // find search text in questions
            roam_data_array_me.questions.forEach((que) => {
                if (que.question_text.indexOf(search_text) != -1) {
                    var div = document.createElement("div");
                    div.className = "question me-search-result me-d-flex-c";
                    if (que.type == "normal") {
                        div.classList.add("normal");
                    } else if (que.type == "mcq") {
                        div.classList.add("mcq");
                    } else if (que.type == "pyq") {
                        div.classList.add("mcq");
                        div.classList.add("pyq");
                    }

                    var span2 = document.createElement("span");
                    span2.className = "me-search-block-main me-cp";
                    var string = que.question_text;
                    string = string.replace(search_text, `==${search_text}==`);

                    span2.innerHTML = convertTextToHTML(string);
                    div.appendChild(span2);
                    span2.addEventListener("click", () => {
                        var b = target.querySelector(".active");
...
// search
function findSearchTextInBlocks(block, target, search_text, page_uid) {
    var string = block.string;
    if (string.indexOf(search_text) != -1) {
        var div = document.createElement("div");
        div.className = "ncert me-search-result me-d-flex-c";

        var span1 = document.createElement("span");
        span1.className = "page-title page-name";
        span1.textContent = getPageTitle(page_uid);
        ...
        span2.addEventListener("click", () => {
            ...
            span2.classList.add("active");
            openAndScrollToBlock(page_uid, block.uid);
        });
    ...
    if (children && children.length) {
        children.forEach((child) => {
            findSearchTextInBlocks(child, target, search_text, page_uid);
        });
    }
}
...
function displayInMainPage(arg) {
    ...
    if (arg == "home") {
        ...
        document.querySelector(".main-page .search-page").classList.remove("hide");
    }
}
displayInMainPage("home");
...
function convertTextToHTML(inputText) {
    ...
    htmlText = htmlText.replace(/==([^=]+)==/g, '<span class="me-search-highlight">$1</span>');
    return htmlText;
}
